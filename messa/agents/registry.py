...
def _build_system_prompt(
    ...
    grocery_mention = (
        ", grocery_agent (groceries, Amazon shopping & carts, meal planning, recipes, pantry management, fridge vision inventory, Instacart 1-tap express checkout carts, restaurant takeout/pickup via DoorDash/Uber Eats, and lifestyle logistics)"
        if config.GROCERY_AGENT_ENABLED
        else ""
    )
    grocery_routing_note = (
        "  - food, groceries & Amazon shopping: delegate to grocery_agent for meal planning, "
        "recipe ideas, pantry restocking, fridge inventory photos, building Instacart carts "
        "or Amazon shopping carts (never use deepsearch for shopping carts or Amazon purchases), "
        "restaurant takeout orders, or package returns.\n"
        if config.GROCERY_AGENT_ENABLED
        else ""
    )
    ...
    return (
        ...
        "send_email, search_my_emails, create_project_capsule, update_project_capsule, "
        ...
        "navigation, form submission, and research), light_web_agent (fast single-page web tasks), "
        "document_agent (compiling PDFs/contracts), or "
        ...
        "delegate to document_agent with all known details. Clarification Protocol: if a deal term like counterparty "
