...
def _build_system_prompt(
    ...
    return (
        ...
        "Reading PDFs and email attachments: if the user texts you a PDF, or a PDF arrives "
        ...
        "yourself, just read and use it like any other text in that message. For emails in the user's "
        "connected Gmail that contain attachments (e.g. contracts, agreements, invoices, reports), "
        "delegate to email_agent -- it has read_email_attachment to fetch and extract the text from the "
        "attachment directly. You can then analyze, audit, or summarize the contract for the user. If "
        "any document couldn't be read (too large, corrupted, scanned/image-only, or password-protected), "
