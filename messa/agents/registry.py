...
def _build_system_prompt(
    ...
    return (
        ...
        "Speed AND cost matter: you have web_search, wikipedia_lookup, fetch_page_text, and "
        "fetch_rendered_page_text as your OWN direct tools (no delegation, no browser session, "
        "answers in a second or a few) -- use them for a plain factual lookup (a fact, news, a "
        "definition, 'who is X', an encyclopedia-shaped question) OR a read-only check (\"does "
        "this page say X yet\", \"has this gone on sale\", \"what does this listing show right "
        "now\") instead of delegating to deepsearch. This matters for real money, not just "
        "speed: deepsearch opens an actual Browserbase browser session that's billed by TIME "
        "HELD OPEN, regardless of how simple the underlying question was -- never delegate to "
        "it for something these four tools can answer. This applies just as much when you're "
        "running because an autonomous routine just fired (a recurring 'check on X' watcher) as "
        "it does in a live conversation -- a routine that re-checks a page every few minutes "
        "should almost never open a fresh browser session to do it.\n"
        ...
        "to show their actual content. This still costs no Browserbase session time (it runs on "
        "a separate reader service), just slightly more latency than a plain fetch.\n"
        ...
        "one of them -- do the comparing with fetch_rendered_page_text across every candidate "
        "first, THEN delegate to deepsearch with a description that already says exactly which "
        "page/option to act on, so it doesn't spend paid session time re-discovering what you "
        "already found.\n\n"
